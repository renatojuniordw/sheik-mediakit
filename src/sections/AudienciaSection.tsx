    ...
    >
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pt-24 pb-16 relative z-10">
        ...
        <div className="mb-12">
          ...
          >
            QUEM CONSOME O <span className="text-cyan drop-shadow-[0_0_15px_rgba(0,240,255,0.3)]">CONTEÚDO.</span>
          </motion.h2>
        </div>
            ...
            >
              <div>
                ...
                <div className="flex gap-10 mt-6 mb-8">
                  <div className="group">
                    ...
                    <p className="font-mono text-[9px] text-white/70 tracking-widest uppercase mt-2">Masculino</p>
                  </div>
                  <div className="group">
                    ...
                    <p className="font-mono text-[9px] text-white/70 tracking-widest uppercase mt-2">Feminino</p>
                  </div>
                </div>
              ...
              <div className="p-4 bg-acid/5 border-l-2 border-acid backdrop-blur-sm">
                <p className="font-mono text-[10px] text-acid font-bold uppercase leading-tight">
                  "Viralização Orgânica: 94,1% das interações vêm de não-seguidores."
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex flex-col gap-6 mt-6">
                {AUDIENCE.age.followers.map(({ range, pct }, i) => (
                  <AudienceBar key={range} label={range} pct={pct} delay={0.1 * i} color="var(--cyan)" />
                ))}
              </div>
              
              <div className="mt-8 pt-6 border-t border-white/5">
                <p className="font-mono text-[9px] text-white/70 uppercase tracking-widest leading-relaxed">
                  Audiência predominante entre <span className="text-white">18 e 34 anos</span> — alto poder de engajamento e consumo.
                </p>
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-4 bg-lead/60 p-8 flex flex-col justify-between">
            <div>
              ...
              <div className="flex flex-col gap-5 mt-6 mb-12">
                {AUDIENCE.cities.followers.slice(0, 5).map(({ city, pct }, i) => (
                  <AudienceBar key={city} label={city} pct={pct} delay={0.2 + (0.05 * i)} color="var(--acid)" />
                ))}
              </div>
              ...
              <div className="flex flex-col gap-3 mt-6">
                {AUDIENCE.countries.map(({ country, pct }) => (
                  <div key={country} className="flex items-center justify-between py-2 border-b border-white/5 group">
                    <span className="text-xs text-white/70 group-hover:text-white transition-colors duration-300 font-mono tracking-tighter">{country}</span>
                    <span className="font-display text-xl text-acid">{pct}%</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-12 group cursor-pointer overflow-hidden relative">
              ...
              <div className="relative z-10 p-4 border border-cyan/20 bg-cyan/5 transition-colors duration-300 group-hover:border-cyan">
                <p className="font-mono text-[8px] text-cyan uppercase tracking-[3px] mb-1">Engajamento Local</p>
                <p className="text-sm text-white font-medium italic">"A maior força do humor regional com alcance nacional."</p>
              </div>
            </div>
          </div>
